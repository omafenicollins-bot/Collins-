...
# ── Important words for yellow highlight ─────────────────────────────────────
IMPORTANT_WORDS = {
...
# ── Job helpers ───────────────────────────────────────────────────────────────
def job_dir(job_id): return OUTPUTS_DIR / job_id
def job_status_path(job_id): return job_dir(job_id) / "status.json"
...
def compile_video_ffmpeg(video_clips, audio_path, output_path, script_text, platform):
    ...
    # 1) Process each clip: scale+crop to target resolution
    processed = []
    ...
    # 3) Build drawtext filter for word-by-word captions
